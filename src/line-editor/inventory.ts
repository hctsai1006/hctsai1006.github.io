...
export class CommandInventory {
  ...
  constructor(manifests: readonly ManifestLike[], options: InventoryOptions = {}) {
     ...
     * Can this be typed?
     *
     * Three different reasons it might not be, and they are not one question:
     *
     *   SHADOWED   another command owns the word. Checked FIRST and regardless
     *              of `includeUnimplemented`, because this is not a gap in the
     *              work — the module is complete. Indexing it under its own
     *              name is what made completion and Get-Help describe the `sl`
     *              easter egg, badge SIMULATED and all, for a token that runs
     *              the native-semantic Set-Location. The alias loop below then
     *              fills the name in for the owner, which is the one true
     *              answer. In `everything` mode the alias still resolves, so
     *              admitting it here would restore the two-answers bug in the
     *              exact mode meant for describing the command set accurately.
     *
     *   SESSION    the registry's own list wins when the caller has one.
     *   STATUS     otherwise implementation status is the honest stand-in:
     *              `declared` means nobody built it and `partial` means it was
     *              built and deliberately held back, and offering either is
     *              offering a name that will not resolve.
     */
    const offerable = (m: ManifestLike): true | string => {
      if (m.shadowedBy !== undefined) return `the token belongs to ${m.shadowedBy}`;
