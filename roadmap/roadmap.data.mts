  ...
  },
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        status: 'partial',
        evidence: [
          { kind: 'export', file: 'tools/conformance.mts', symbol: 'classifyDeltaProof' },
          { kind: 'test', file: 'tests/unit/conformance-coverage.test.mts', name: 'fails the build when the named case is gone' },
          { kind: 'test', file: 'tests/unit/conformance-coverage.test.mts', name: 'fails the build when "verified" names nothing' },
          { kind: 'json', file: 'compat/deltas/7.6.5__7.7.0-preview.4.json', path: 'changes.0.emulated' },
          { kind: 'json', file: 'compat/deltas/7.6.5__7.7.0-preview.4.json', path: 'summary.emulated' },
          { kind: 'json', file: 'tests/conformance/report.json', path: 'deltaProof.proven' },
          { kind: 'code', file: 'tools/compat-curation.mts', pattern: 'is "verified" but names no conformance case' },
        ],
      },
      {
        id: '3.6',
  ...
  },
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        status: 'done',
        evidence: [
          { kind: 'export', file: 'src/input/textarea.ts', symbol: 'TextareaInputAdapter' },
          { kind: 'export', file: 'src/input/textarea.ts', symbol: 'InputAdapter' },
          { kind: 'export', file: 'src/input/textarea.ts', symbol: 'TextareaLike' },
          { kind: 'test', file: 'tests/unit/line-editor-input.test.mts', name: 'writes the core onto the surface after every handled key' },
          { kind: 'test', file: 'tests/unit/line-editor-input.test.mts', name: 'takes a native edit back into the core wholesale' },
          { kind: 'test', file: 'tests/unit/line-editor-input.test.mts', name: 'is satisfied by a real HTMLTextAreaElement, as lib.dom declares one' },
        ],
      },
      {
        id: '5.3',
      ...
      },
      {
        ...
        evidence: [
          ...
          { kind: 'export', file: 'src/input/ime.ts', symbol: 'imeGuardLeg' },
          { kind: 'export', file: 'src/input/ime.ts', symbol: 'IME_SENTINEL_KEYCODE' },
          { kind: 'test', file: 'tests/unit/line-editor-input.test.mts', name: 'stops the keydown that starts a composition, which only the 229 sentinel sees' },
          { kind: 'test', file: 'tests/unit/line-editor-input.test.mts', name: 'stops a mid-composition keydown that reports neither flag, which only the sticky leg sees' },
          { kind: 'test', file: 'tests/unit/line-editor-input.test.mts', name: 'stops a keydown inside a composition this adapter never saw start, which only isComposing sees' },
          { kind: 'test', file: 'tests/unit/line-editor-input.test.mts', name: 'guards a composition keystroke on every engine profile, with and without the compositionstart' },
          { kind: 'test', file: 'tests/unit/line-editor-input.test.mts', name: 'survives Safari sending the confirming keydown after compositionend' },
          { kind: 'test', file: 'tests/unit/line-editor-input.test.mts', name: 'hands back every key between compositionstart and compositionend' },
          { kind: 'absent', glob: 'src/line-editor/**/*.ts', pattern: 'keyCode' },
        ],
      },
    ],
    ...
    tasks: [
      {
        ...
        status: 'done',
        evidence: [
          { kind: 'export', file: 'src/providers/types.ts', symbol: 'ItemProvider' },
          { kind: 'export', file: 'src/providers/types.ts', symbol: 'ContainerProvider' },
          { kind: 'export', file: 'src/providers/types.ts', symbol: 'NavigationProvider' },
          { kind: 'export', file: 'src/providers/types.ts', symbol: 'ContentProvider' },
          { kind: 'export', file: 'src/storage/vfs.ts', symbol: 'ForeignDrives' },
          { kind: 'test', file: 'tests/unit/providers.test.mts', name: 'makes the four session-state providers containers but NOT navigation providers' },
          { kind: 'test', file: 'tests/unit/providers.test.mts', name: 'can express a refusal for a provider that implements neither layer' },
        ],
      },
      {
        id: '10.2',
        ...
        status: 'done',
        evidence: [
          { kind: 'export', file: 'src/providers/session-state.ts', symbol: 'SessionStateProvider' },
          { kind: 'export', file: 'src/providers/filesystem.ts', symbol: 'FileSystemProvider' },
          { kind: 'export', file: 'src/providers/registry.ts', symbol: 'ProviderRegistry' },
          { kind: 'script', name: 'capture:providers' },
          { kind: 'test', file: 'tests/unit/providers.test.mts', name: 'lists Env:/ which is the acceptance criterion' },
          { kind: 'test', file: 'tests/unit/providers.test.mts', name: 'reproduces the measured table: clearing writes null, and null means different things' },
          { kind: 'test', file: 'tests/unit/providers.test.mts', name: 'reproduces the collated order pwsh returns, in every session-state drive' },
          { kind: 'test', file: 'tests/unit/providers.test.mts', name: 'teaches the SAME resolver about provider drives instead of adding a second' },
        ],
      },
      {
        id: '10.3',
        ...
        status: 'todo',
        evidence: [
          { kind: 'absent', glob: 'src/providers/**/*.ts', pattern: 'PortfolioProvider|ProcessProvider|PackageProvider|BrowserProvider' },
          { kind: 'no-files', glob: 'src/providers/portfolio.ts', within: 'src/providers/*.ts' },
        ],
      },
      {
        id: '10.4',
  ...
  // --------------------------------------------------------------- Compatibility
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        status: 'done',
        evidence: [
          { kind: 'export', file: 'tools/conformance.mts', symbol: 'classifyProfileCoverage' },
          { kind: 'json', file: 'tests/conformance/report.json', path: 'profileCoverage.1.commands.verifiedPercent' },
          { kind: 'test', file: 'tests/unit/conformance-coverage.test.mts', name: 'never lets the evidence touch the denominator' },
          { kind: 'test', file: 'tests/unit/conformance-coverage.test.mts', name: 'awards nothing from a capture of a different version, however much evidence there is' },
          { kind: 'test', file: 'tests/conformance/conformance.test.mts', name: 'reports a coverage number for every published profile, not just the one with a fixture' },
          { kind: 'code', file: 'compat/explorer.html', pattern: 'How much of this has been checked against a real PowerShell' },
